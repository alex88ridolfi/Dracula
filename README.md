...
* If necessary, put an EFAC in your timfile such that this step also results in a reduced chi-2 of ~1.

* Epochs can be joined together by removing JUMPs from the timfile. Try doing this between nearby epochs, by removing two successive JUMP statements and inserting a "PHASE N" (where N is some integer number of phase wraps) between them. Some value of N (maybe 0) will hopefully result in a reduced chi2 ~1. If changing N by +/-1 should give a reduced chi2 that is considerably larger than 1, then you have an unambiguous connection, i.e., you connected that gap. In this case, move to another gap where you feel you can now get an unambiguous solution.
